...
export default function MasterOrganizationTypesPage() {
	...
	const handleAdd = async (data: { nameEn: string, nameBn: string }): Promise<boolean | null> => {
		try {
			...
			toast.error({ title: 'Error', description: 'Failed to add organization type.' });
			return null;
		}
	};
	...
	const handleUpdate = async (item: ICommonMasterData): Promise<boolean | null> => {
		try {
			...
			toast.error({ title: 'Error', description: 'Failed to update organization type.' });
			return null;
		}
	};
	...
	const handleDelete = async (id: string): Promise<boolean> => {
		try {
			...
			toast.success({ title: 'Success', description: 'Organization type deleted successfully.' });
			loadItems(meta.page, debouncedSearch);
			return true;
		} catch (error) {
			console.error('Failed to delete item', error);
			toast.error({ title: 'Error', description: 'Failed to delete organization type.' });
			return false;
		}
	};
