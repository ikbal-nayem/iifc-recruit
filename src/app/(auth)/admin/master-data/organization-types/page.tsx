...
export default function MasterOrganizationTypesPage() {
	...
	const handleAdd = async (data: { nameEn: string, nameBn: string }): Promise<boolean | null> => {
		try {
			...
			toast({ title: 'Error', description: 'Failed to add organization type.', variant: 'danger' });
			return null;
		}
	};
	...
	const handleUpdate = async (item: ICommonMasterData): Promise<boolean | null> => {
		try {
			const updatedItem = await MasterDataService.organizationType.update(item);
			setItems(items.map((i) => (i?.id === item?.id ? updatedItem?.body : i)));
			toast({ description: updatedItem?.message, variant: 'success' });
			return true;
		} catch (error) {
			console.error('Failed to update item', error);
			toast({ title: 'Error', description: 'Failed to update organization type.', variant: 'danger' });
			return null;
		}
	};
	...
	const handleDelete = async (id: string): Promise<boolean> => {
		try {
			await MasterDataService.organizationType.delete(id);
			toast({ title: 'Success', description: 'Organization type deleted successfully.', variant: 'success' });
			loadItems(meta.page, debouncedSearch);
			return true;
		} catch (error) {
			console.error('Failed to delete item', error);
			toast({ title: 'Error', description: 'Failed to delete organization type.', variant: 'danger' });
			return false;
		}
	};
