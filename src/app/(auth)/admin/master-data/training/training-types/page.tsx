...
export default function MasterTrainingTypesPage() {
	...
	const loadItems = useCallback(
		async (page: number, search: string) => {
			...
			try {
				...
				const response = await MasterDataService.trainingType.getList(payload);
	...
	const handleAdd = async (name: string): Promise<boolean | null> => {
		try {
			const resp = await MasterDataService.trainingType.add({ name, isActive: true });
	...
	const handleUpdate = async (item: ICommonMasterData): Promise<boolean | null> => {
		try {
			const updatedItem = await MasterDataService.trainingType.update(item);
	...
	const handleDelete = async (id: string): Promise<boolean> => {
		try {
			await MasterDataService.trainingType.delete(id);
			toast({ title: 'Success', description: 'Training type deleted successfully.', variant: 'success' });
			loadItems(meta.page, debouncedSearch);
			return true;
		} catch (error) {
