...
export default function MasterCertificationsPage() {
	...
	const handleAdd = async (data: { nameEn: string, nameBn: string }): Promise<boolean | null> => {
		try {
			const resp = await MasterDataService.certification.add({ ...data, isActive: true });
			toast({ description: resp.message, variant: 'success' });
			loadItems(meta.page, debouncedSearch);
			return true;
		} catch (error) {
			console.error('Failed to add item', error);
			toast({ title: 'Error', description: 'Failed to add certification.', variant: 'danger' });
			return null;
		}
	};
	...
	const handleUpdate = async (item: ICommonMasterData): Promise<boolean | null> => {
		try {
			...
			toast({ title: 'Error', description: 'Failed to update certification.', variant: 'danger' });
			return null;
		}
	};
	...
	const handleDelete = async (id: string): Promise<boolean> => {
		try {
			...
			toast({ title: 'Success', description: 'Certification deleted successfully.', variant: 'success' });
			loadItems(meta.page, debouncedSearch);
			return true;
		} catch (error) {
			console.error('Failed to delete item', error);
			toast({ title: 'Error', description: 'Failed to delete certification.', variant: 'danger' });
			return false;
		}
	};
