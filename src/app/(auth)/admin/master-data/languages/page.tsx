...
export default function MasterLanguagesPage() {
	...
	const handleAdd = async (data: { nameEn: string, nameBn: string }): Promise<boolean | null> => {
		try {
			const resp = await MasterDataService.language.add({ ...data, active: true });
			toast.success({ description: resp.message });
			loadItems(meta.page, debouncedSearch);
			return true;
		} catch (error) {
			console.error('Failed to add item', error);
			toast.error({ title: 'Error', description: 'Failed to add language.' });
			return null;
		}
	};
	...
	const handleUpdate = async (item: ICommonMasterData): Promise<boolean | null> => {
		try {
			const updatedItem = await MasterDataService.language.update(item);
			setItems(items.map((i) => (i?.id === item?.id ? updatedItem?.body : i)));
			toast.success({ description: updatedItem?.message });
			return true;
		} catch (error) {
			console.error('Failed to update item', error);
			toast.error({ title: 'Error', description: 'Failed to update language.' });
			return null;
		}
	};
	...
	const handleDelete = async (id: string): Promise<boolean> => {
		try {
			await MasterDataService.language.delete(id);
			toast.success({ title: 'Success', description: 'Language deleted successfully.' });
			loadItems(meta.page, debouncedSearch);
			return true;
		} catch (error) {
			console.error('Failed to delete item', error);
			toast.error({ title: 'Error', description: 'Failed to delete language.' });
			return false;
		}
	};
