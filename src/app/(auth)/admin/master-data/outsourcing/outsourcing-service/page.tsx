...
export default function MasterOutsourcingServicePage() {
	...
	const loadItems = useCallback(
		async (page: number, search: string) => {
			...
			try {
				...
				toast({
					title: 'Error',
					description: 'Failed to load outsourcing services.',
					variant: 'danger',
				});
			} finally {
				setIsLoading(false);
	...
	const handleAdd = async (name: string): Promise<boolean | null> => {
		try {
			const resp = await MasterDataService.outsourcingService.add({ name, isActive: true });
			toast({ description: resp.message, variant: 'success' });
			loadItems(meta.page, debouncedSearch);
			return true;
		} catch (error) {
			console.error('Failed to add item', error);
			toast({ title: 'Error', description: 'Failed to add outsourcing service.', variant: 'danger' });
			return null;
		}
	};
	...
	const handleUpdate = async (item: ICommonMasterData): Promise<boolean | null> => {
		try {
			const updatedItem = await MasterDataService.outsourcingService.update(item);
			setItems(items.map((i) => (i?.id === item?.id ? updatedItem?.body : i)));
			toast({ description: updatedItem?.message, variant: 'success' });
			return true;
		} catch (error) {
			console.error('Failed to update item', error);
			toast({ title: 'Error', description: 'Failed to update outsourcing service.', variant: 'danger' });
			return null;
		}
	};
	...
	const handleDelete = async (id: string): Promise<boolean> => {
		try {
			...
			toast({ title: 'Success', description: 'Outsourcing service deleted successfully.', variant: 'success' });
			loadItems(meta.page, debouncedSearch);
			return true;
		} catch (error) {
			console.error('Failed to delete item', error);
			toast({ title: 'Error', description: 'Failed to delete outsourcing service.', variant: 'danger' });
			return false;
		}
	};
