...
export default function MasterIndustryTypesPage() {
	...
	const handleUpdate = async (item: ICommonMasterData): Promise<boolean | null> => {
		try {
			const updatedItem = await MasterDataService.industryType.update(item);
	...
	const handleDelete = async (id: string): Promise<boolean> => {
		try {
			await MasterDataService.industryType.delete(id);
			toast({ title: 'Success', description: 'Industry type deleted successfully.', variant: 'success' });
			loadItems(meta.page, debouncedSearch);
			return true;
		} catch (error) {
