...
export default function MasterPositionLevelsPage() {
	...
	const handleAdd = async (data: { nameEn: string; nameBn: string }): Promise<boolean | null> => {
		try {
			const resp = await MasterDataService.positionLevel.add({ ...data, active: true });
	...
	const handleDelete = async (id: string): Promise<boolean> => {
		try {
			await MasterDataService.positionLevel.delete(id);
			toast({ title: 'Success', description: 'Position level deleted successfully.', variant: 'success' });
			loadItems(meta.page, debouncedSearch);
			return true;
		} catch (error) {
