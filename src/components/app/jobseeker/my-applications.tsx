  ...
  const columns: ColumnDef<ApplicationWithJob>[] = [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Link href={`/jobseeker/jobs/${application.jobId}`} className="font-medium text-primary hover:underline">
  ...
  const renderMobileCard = (application: ApplicationWithJob) => (
    <Card key={application.id} className="mb-4 glassmorphism">
      <div className="p-4 space-y-2">
        <Link href={`/jobseeker/jobs/${application.jobId}`} className="font-semibold text-lg text-primary hover:underline">
          {application.job?.title}
        </Link>
