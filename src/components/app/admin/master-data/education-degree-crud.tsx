...
interface EducationDegreeFormProps {
	isOpen: boolean;
	onClose: () => void;
	onSubmit: (data: IEducationDegree | Omit<IEducationDegree, 'id'>) => Promise<boolean>;
	initialData?: IEducationDegree;
	degreeLevels: ICommonMasterData[];
	noun: string;
}

function EducationDegreeForm({
	isOpen,
	onClose,
	onSubmit,
	initialData,
		...
		<Dialog open={isOpen} onOpenChange={onClose}>
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>
						{initialData
							? 'Update the details of the degree.'
				...
				<Form {...form}>
					<form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-4 py-4'>
						<FormAutocomplete
							...
							initialLabel={initialData?.degreeLevel?.nameEn}
						/>
						<FormInput
							control={form.control}
							name='nameEn'
...
interface EducationDegreeCrudProps {
	title: string;
	description: string;
	noun: string;
	items: IEducationDegree[];
	meta: IMeta;
	isLoading: boolean;
	degreeLevels: ICommonMasterData[];
	onAdd: (item: Omit<IEducationDegree, 'id'>) => Promise<boolean>;
	onUpdate: (item: IEducationDegree) => Promise<boolean>;
...
export function EducationDegreeCrud({
	...
	const [editingItem, setEditingItem] = useState<IEducationDegree | undefined>(undefined);
	const [itemToDelete, setItemToDelete] = useState<IEducationDegree | null>(null);

	const handleOpenForm = (item?: IEducationDegree) => {
		setEditingItem(item);
		setIsFormOpen(true);
	};
	...
	const handleFormSubmit = async (data: IEducationDegree | Omit<IEducationDegree, 'id'>) => {
		if ('id' in data) {
			return onUpdate(data as IEducationDegree);
		}
		return onAdd(data as Omit<IEducationDegree, 'id'>);
	};

	const handleToggleActive = async (item: IEducationDegree) => {
		if (!item.id) return;
