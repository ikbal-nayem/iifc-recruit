...
interface OutsourcingZoneFormProps {
	isOpen: boolean;
	onClose: () => void;
	onSubmit: (data: FormValues, id?: string) => Promise<boolean | null>;
	initialData?: IOutsourcingZone;
	noun: string;
}

function OutsourcingZoneForm({
	isOpen,
	onClose,
	onSubmit,
	initialData,
	noun,
}: OutsourcingZoneFormProps) {
...
interface OutsourcingZoneCrudProps {
	title: string;
	description: string;
	noun: string;
	items: IOutsourcingZone[];
	meta: IMeta;
	isLoading: boolean;
	onAdd: (data: { nameEn: string; nameBn: string }) => Promise<boolean | null>;
	onUpdate: (item: IOutsourcingZone) => Promise<boolean | null>;
	onDelete: (id: string) => Promise<boolean>;
	onPageChange?: (page: number) => void;
	onSearch: (query: string) => void;
}

export function OutsourcingZoneCrud({
	title,
	description,
	noun,
	items,
	...
	onPageChange,
	onSearch,
}: OutsourcingZoneCrudProps) {
	const { toast } = useToast();
	const [isSubmitting, setIsSubmitting] = useState<string | null>(null);
	const [isFormOpen, setIsFormOpen] = useState(false);
	const [editingItem, setEditingItem] = useState<IOutsourcingZone | undefined>(undefined);

	const handleOpenForm = (item?: IOutsourcingZone) => {
		setEditingItem(item);
		setIsFormOpen(true);
	};
	...
	const handleToggleActive = async (item: IOutsourcingZone) => {
		if (!item.id) return;
