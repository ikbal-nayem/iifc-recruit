import type { Candidate, Job, Application, Activity } from './types';
import { subDays, format, subMonths, addDays } from 'date-fns';

export const candidates: Candidate[] = [
  {
    id: 'c1',
    personalInfo: {
      firstName: 'Alice',
      lastName: 'Johnson',
      name: 'Alice Johnson',
      email: 'alice.j@example.com',
      phone: '123-456-7890',
      address: {
        line1: '123 Tech Avenue',
        upazila: 'Gulshan',
        district: 'Dhaka',
        division: 'Dhaka',
      },
      avatar: 'https://picsum.photos/seed/alice/100/100',
      headline: 'Senior Frontend Developer with 8+ years of experience',
    },
    academicInfo: [
      {
        degree: 'B.S. in Computer Science',
        institution: 'Stanford University',
        graduationYear: 2016,
        certificateUrls: ['/certs/cert1.pdf', '/certs/cert2.pdf']
      },
    ],
    professionalInfo: [
      {
        company: 'TechCorp',
        role: 'Senior Frontend Developer',
        duration: '2019 - Present',
        responsibilities: ['Led development of a new design system', 'Mentored junior developers'],
      },
       {
        company: 'Innovate LLC',
        role: 'Frontend Developer',
        duration: '2016 - 2019',
        responsibilities: ['Built and maintained client-facing web applications', 'Collaborated with UI/UX designers'],
      },
    ],
    skills: ['React', 'TypeScript', 'Next.js', 'GraphQL', 'Node.js', 'Figma'],
    certifications: [
        { name: 'Certified Kubernetes Application Developer', issuingOrganization: 'The Linux Foundation', issueDate: '2022-01-15' },
    ],
    languages: [
        { name: 'English', proficiency: 'Native' },
        { name: 'Spanish', proficiency: 'Advanced' },
    ],
    publications: [
        { title: 'The Rise of Server-Side Rendering', publisher: 'Smashing Magazine', publicationDate: '2023-05-20', url: 'https://example.com' },
    ],
    awards: [
        { name: 'Developer of the Year', awardingBody: 'TechCorp', dateReceived: '2021-12-20' },
    ],
    resumeUrl: '/resumes/alice_johnson_resume.pdf',
    status: 'Active',
  },
  {
    id: 'c2',
    personalInfo: {
      firstName: 'Bob',
      lastName: 'Smith',
      name: 'Bob Smith',
      email: 'bob.s@example.com',
      phone: '234-567-8901',
       address: {
        line1: '456 Innovation Drive',
        upazila: 'Chittagong Sadar',
        district: 'Chittagong',
        division: 'Chittagong',
      },
      avatar: 'https://picsum.photos/seed/bob/100/100',
      headline: 'Product Manager driving innovation and growth',
    },
    academicInfo: [
      {
        degree: 'MBA',
        institution: 'Columbia Business School',
        graduationYear: 2018,
      },
    ],
    professionalInfo: [
      {
        company: 'Solutions Inc.',
        role: 'Product Manager',
        duration: '2018 - Present',
        responsibilities: ['Defined product roadmap for B2B SaaS platform', 'Conducted market research and user interviews'],
      },
    ],
    skills: ['Product Management', 'Agile', 'JIRA', 'Market Research', 'Roadmap Planning'],
    certifications: [],
    languages: [{ name: 'English', proficiency: 'Native' }],
    publications: [],
    awards: [],
    resumeUrl: '/resumes/bob_smith_resume.pdf',
    status: 'Passive',
  },
  {
    id: 'c3',
    personalInfo: {
      firstName: 'Charlie',
      lastName: 'Brown',
      name: 'Charlie Brown',
      email: 'charlie.b@example.com',
      phone: '345-678-9012',
      address: {
        line1: '789 ML Lane',
        upazila: 'Sylhet Sadar',
        district: 'Sylhet',
        division: 'Sylhet',
      },
      avatar: 'https://picsum.photos/seed/charlie/100/100',
      headline: 'Data Scientist passionate about machine learning',
    },
    academicInfo: [
       {
        degree: 'M.S. in Data Science',
        institution: 'University of Texas at Austin',
        graduationYear: 2020,
      },
    ],
    professionalInfo: [
      {
        company: 'DataWiz',
        role: 'Data Scientist',
        duration: '2020 - Present',
        responsibilities: ['Developed predictive models for customer churn', 'Built data visualization dashboards in Tableau'],
      },
    ],
    skills: ['Python', 'TensorFlow', 'PyTorch', 'SQL', 'Tableau', 'Machine Learning'],
    certifications: [],
    languages: [{ name: 'English', proficiency: 'Native' }],
    publications: [],
    awards: [],
    resumeUrl: '/resumes/charlie_brown_resume.pdf',
    status: 'Active',
  },
    {
    id: 'c4',
    personalInfo: {
      firstName: 'Diana',
      lastName: 'Prince',
      name: 'Diana Prince',
      email: 'diana.p@example.com',
      phone: '456-789-0123',
      address: {
        line1: '101 Design Street',
        upazila: 'Rajshahi Sadar',
        district: 'Rajshahi',
        division: 'Rajshahi',
      },
      avatar: 'https://picsum.photos/seed/diana/100/100',
      headline: 'UX/UI Designer creating intuitive user experiences',
    },
    academicInfo: [
      {
        degree: 'B.A. in Graphic Design',
        institution: 'Rhode Island School of Design',
        graduationYear: 2019,
      },
    ],
    professionalInfo: [
       {
        company: 'Creative Minds',
        role: 'UX/UI Designer',
        duration: '2019 - Present',
        responsibilities: ['Designed user flows and wireframes', 'Conducted usability testing sessions'],
      },
    ],
    skills: ['Figma', 'Sketch', 'Adobe XD', 'User Research', 'Prototyping'],
    certifications: [],
    languages: [{ name: 'English', proficiency: 'Native' }],
    publications: [],
    awards: [],
    resumeUrl: '/resumes/diana_prince_resume.pdf',
    status: 'Hired',
  },
];

export const jobs: Job[] = [
  {
    id: 'j1',
    title: 'Assistant Director',
    department: 'Bangladesh Bank',
    location: 'Dhaka',
    type: 'Full-time',
    salaryRange: 'BDT 55,000 - 65,000',
    description: 'Bangladesh Bank is seeking qualified candidates for the position of Assistant Director to contribute to the central banking operations.',
    responsibilities: ['Assisting in monetary policy formulation.', 'Conducting economic research and analysis.', 'Supervising financial institutions.'],
    requirements: ['Masters degree in Economics, Finance, or related field.', 'Strong analytical and communication skills.', 'Knowledge of banking regulations.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 5), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 25), 'yyyy-MM-dd'),
  },
  {
    id: 'j2',
    title: 'Medical Officer',
    department: 'Ministry of Health and Family Welfare',
    location: 'Multiple Locations',
    type: 'Full-time',
    salaryRange: 'BDT 40,000 - 50,000',
    description: 'The Ministry of Health is looking for dedicated Medical Officers to provide healthcare services in government hospitals and health complexes.',
    responsibilities: ['Providing medical treatment to patients.', 'Managing public health programs.', 'Maintaining medical records.'],
    requirements: ['MBBS degree from a recognized university.', 'Valid registration with BMDC.', 'Experience in clinical practice.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 12), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 18), 'yyyy-MM-dd'),
  },
  {
    id: 'j3',
    title: 'Project Engineer (Civil)',
    department: 'Roads and Highways Department',
    location: 'Dhaka',
    type: 'Contract',
    salaryRange: 'BDT 70,000 - 85,000',
    description: 'Seeking a skilled Civil Engineer for infrastructure development projects, including bridges and highways.',
    responsibilities: ['Supervising construction projects.', 'Ensuring quality control and safety standards.', 'Preparing project reports and documentation.'],
    requirements: ['B.Sc. in Civil Engineering.', 'Minimum 5 years of experience in road or bridge construction.', 'Proficiency in AutoCAD and project management software.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 20), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 10), 'yyyy-MM-dd'),
  },
  {
    id: 'j4',
    title: 'Upazila Nirbahi Officer (UNO)',
    department: 'Ministry of Public Administration',
    location: 'Various Upazilas',
    type: 'Full-time',
    salaryRange: 'As per National Pay Scale',
    description: 'A prestigious role in the Bangladesh Civil Service responsible for administration and development at the Upazila level.',
    responsibilities: ['Implementing government policies.', 'Coordinating with various government departments.', 'Overseeing local development projects.'],
    requirements: ['Qualified in Bangladesh Civil Service (BCS) examination.', 'Strong leadership and administrative skills.', 'Commitment to public service.'],
    status: 'Closed',
    postedDate: format(subMonths(new Date(), 2), 'yyyy-MM-dd'),
    applicationDeadline: format(subMonths(new Date(), 1), 'yyyy-MM-dd'),
  },
  {
    id: 'j5',
    title: 'IT Officer',
    department: 'ICT Division',
    location: 'Dhaka',
    type: 'Full-time',
    salaryRange: 'BDT 45,000 - 55,000',
    description: 'The ICT Division requires a skilled IT Officer to manage and maintain government IT infrastructure and support digital initiatives.',
    responsibilities: ['Network administration and security.', 'System maintenance and troubleshooting.', 'Supporting e-Governance projects.'],
    requirements: ['B.Sc. in Computer Science or IT.', 'Experience with network hardware, servers, and cybersecurity best practices.', 'Relevant certifications (e.g., CCNA, MCSA) are a plus.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 8), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 22), 'yyyy-MM-dd'),
  },
  {
    id: 'j6',
    title: 'Customs Officer',
    department: 'National Board of Revenue (NBR)',
    location: 'Chittagong',
    type: 'Full-time',
    salaryRange: 'As per National Pay Scale',
    description: 'An important role in revenue collection and trade facilitation at the ports and borders of Bangladesh.',
    responsibilities: ['Assessing and collecting customs duties and taxes.', 'Preventing smuggling and illegal trade.', 'Inspecting cargo and documentation.'],
    requirements: ['Qualified through the BCS (Customs and Excise) cadre examination.', 'Strong integrity and attention to detail.', 'Knowledge of customs laws and tariffs.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 25), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 5), 'yyyy-MM-dd'),
  },
  {
    id: 'j7',
    title: 'Power Plant Manager',
    department: 'Bangladesh Power Development Board (BPDB)',
    location: 'Gazipur',
    type: 'Full-time',
    salaryRange: 'BDT 90,000 - 110,000',
    description: 'Experienced manager sought to oversee the operations and maintenance of a government-owned power plant.',
    responsibilities: ['Managing daily power plant operations.', 'Ensuring compliance with safety and environmental regulations.', 'Leading a team of engineers and technicians.'],
    requirements: ['B.Sc. in Electrical or Mechanical Engineering.', '10+ years of experience in power plant management.', 'Strong technical and leadership skills.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 3), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 27), 'yyyy-MM-dd'),
  },
  {
    id: 'j8',
    title: 'Lecturer (Computer Science)',
    department: 'Ministry of Education',
    location: 'Various Govt. Colleges',
    type: 'Full-time',
    salaryRange: 'As per National Pay Scale',
    description: 'Seeking passionate educators to teach Computer Science at the higher secondary and undergraduate levels in government colleges.',
    responsibilities: ['Delivering lectures and conducting lab sessions.', 'Developing course materials and assessing students.', 'Participating in academic committees.'],
    requirements: ['Masters in Computer Science from a reputable university.', 'Strong academic record.', 'Prior teaching experience is preferred.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 15), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 15), 'yyyy-MM-dd'),
  },
    {
    id: 'j9',
    title: 'Urban Planner',
    department: 'Rajdhani Unnayan Kartripakkha (RAJUK)',
    location: 'Dhaka',
    type: 'Full-time',
    salaryRange: 'BDT 60,000 - 75,000',
    description: 'Join RAJUK to contribute to the planned development of Dhaka city, focusing on sustainable urban growth and infrastructure.',
    responsibilities: ['Developing master plans and detailed area plans.', 'Reviewing development proposals.', 'Conducting urban research and data analysis.'],
    requirements: ['Bachelors or Masters in Urban and Regional Planning (URP).', 'Knowledge of GIS software and planning regulations.', 'Experience in a government planning agency is a plus.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 22), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 8), 'yyyy-MM-dd'),
  },
  {
    id: 'j10',
    title: 'Scientific Officer',
    department: 'Bangladesh Council of Scientific and Industrial Research (BCSIR)',
    location: 'Dhaka',
    type: 'Full-time',
    salaryRange: 'As per BCSIR Pay Scale',
    description: 'A research-oriented position for individuals passionate about scientific innovation and industrial development.',
    responsibilities: ['Conducting research in a specified scientific field.', 'Publishing research findings.', 'Collaborating with industries for technology transfer.'],
    requirements: ['M.Sc. or PhD in a relevant scientific discipline (e.g., Chemistry, Physics, Biology).', 'Proven research capabilities and publications.', 'Strong laboratory skills.'],
    status: 'Archived',
    postedDate: format(subMonths(new Date(), 3), 'yyyy-MM-dd'),
    applicationDeadline: format(subMonths(new Date(), 2), 'yyyy-MM-dd'),
  },
  {
    id: 'j11',
    title: 'Port Operations Officer',
    department: 'Chittagong Port Authority',
    location: 'Chittagong',
    type: 'Full-time',
    salaryRange: 'BDT 50,000 - 60,000',
    description: 'Assist in the management of day-to-day operations at the nation\'s primary seaport.',
    responsibilities: ['Coordinating vessel traffic and berthing.', 'Managing cargo handling and storage.', 'Ensuring port safety and security protocols are followed.'],
    requirements: ['Masters in Maritime Science, Management, or a related field.', 'Experience in port or shipping industry.', 'Excellent organizational and communication skills.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 7), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 23), 'yyyy-MM-dd'),
  },
  {
    id: 'j12',
    title: 'Agronomist',
    department: 'Bangladesh Agricultural Research Institute (BARI)',
    location: 'Gazipur',
    type: 'Full-time',
    salaryRange: 'As per BARI Pay Scale',
    description: 'Conduct research to improve crop yield, quality, and resistance to diseases for ensuring national food security.',
    responsibilities: ['Designing and conducting field experiments.', 'Developing improved crop varieties.', 'Disseminating research findings to farmers.'],
    requirements: ['PhD or M.Sc. in Agronomy or a related agricultural science.', 'Strong background in agricultural research.', 'Fieldwork experience is essential.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 30), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 0), 'yyyy-MM-dd'),
  },
    {
    id: 'j13',
    title: 'Assistant Commissioner (Land)',
    department: 'Ministry of Land',
    location: 'Various Districts',
    type: 'Full-time',
    salaryRange: 'As per National Pay Scale',
    description: 'A key administrative role in land management and revenue collection at the upazila level.',
    responsibilities: ['Managing land records and mutation.', 'Resolving land disputes.', 'Administering government-owned (khas) land.'],
    requirements: ['Qualified through the BCS (Admin) cadre examination.', 'Strong understanding of land laws in Bangladesh.', 'Excellent conflict resolution skills.'],
    status: 'Closed',
    postedDate: format(subMonths(new Date(), 4), 'yyyy-MM-dd'),
    applicationDeadline: format(subMonths(new Date(), 3), 'yyyy-MM-dd'),
  },
  {
    id: 'j14',
    title: 'Telecommunication Engineer',
    department: 'Bangladesh Telecommunication Company Ltd (BTCL)',
    location: 'Dhaka',
    type: 'Full-time',
    salaryRange: 'BDT 65,000 - 80,000',
    description: 'Work on the expansion and maintenance of the national telecommunication network.',
    responsibilities: ['Planning and implementing telecom network upgrades.', 'Monitoring network performance.', 'Troubleshooting and resolving technical issues.'],
    requirements: ['B.Sc. in Electrical & Electronic Engineering (EEE) or Telecommunications Engineering.', 'Experience with fiber optics, microwave, and switching systems.', 'Knowledge of modern telecom protocols.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 18), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 12), 'yyyy-MM-dd'),
  },
  {
    id: 'j15',
    title: 'Fisheries Officer',
    department: 'Department of Fisheries',
    location: 'Coastal Districts',
    type: 'Full-time',
    salaryRange: 'As per National Pay Scale',
    description: 'Promote sustainable aquaculture and manage fisheries resources in your assigned area.',
    responsibilities: ['Providing technical assistance to fish farmers.', 'Implementing government fisheries projects.', 'Conserving fish biodiversity.'],
    requirements: ['B.Sc. or M.Sc. in Fisheries or a related field.', 'Practical knowledge of aquaculture techniques.', 'Strong communication skills to work with local communities.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 40), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), -10), 'yyyy-MM-dd'),
  },
  {
    id: 'j16',
    title: 'Public Relations Officer',
    department: 'Press Information Department (PID)',
    location: 'Dhaka',
    type: 'Full-time',
    salaryRange: 'BDT 40,000 - 50,000',
    description: 'Manage media relations and public communications for various government ministries and departments.',
    responsibilities: ['Writing and distributing press releases.', 'Organizing press conferences.', 'Monitoring media coverage.'],
    requirements: ['Masters in Mass Communication & Journalism or Public Relations.', 'Excellent writing skills in both Bengali and English.', 'Experience working with media outlets.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 1), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 29), 'yyyy-MM-dd'),
  },
  {
    id: 'j17',
    title: 'Textile Engineer',
    department: 'Department of Textiles',
    location: 'Dhaka',
    type: 'Full-time',
    salaryRange: 'BDT 50,000 - 65,000',
    description: 'Contribute to the policy-making and development of the vital textile sector of Bangladesh.',
    responsibilities: ['Inspecting textile mills for quality and compliance.', 'Providing technical guidance to industry stakeholders.', 'Working on skill development projects.'],
    requirements: ['B.Sc. in Textile Engineering.', 'Knowledge of textile manufacturing processes and quality standards.', 'Good analytical and interpersonal skills.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 9), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 21), 'yyyy-MM-dd'),
  },
  {
    id: 'j18',
    title: 'Assistant Secretary',
    department: 'Ministry of Foreign Affairs',
    location: 'Dhaka',
    type: 'Full-time',
    salaryRange: 'As per National Pay Scale',
    description: 'Represent Bangladesh on the global stage as a diplomat in the Bangladesh Foreign Service.',
    responsibilities: ['Assisting in diplomatic relations and negotiations.', 'Providing consular services.', 'Analyzing international affairs.'],
    requirements: ['Qualified through the BCS (Foreign Affairs) cadre examination.', 'Excellent command of English and strong interpersonal skills.', 'Deep knowledge of international relations and history.'],
    status: 'Closed',
    postedDate: format(subMonths(new Date(), 6), 'yyyy-MM-dd'),
    applicationDeadline: format(subMonths(new Date(), 5), 'yyyy-MM-dd'),
  },
  {
    id: 'j19',
    title: 'Archaeologist',
    department: 'Department of Archaeology',
    location: 'Bogura',
    type: 'Full-time',
    salaryRange: 'As per DoA Pay Scale',
    description: 'Work on the excavation, preservation, and research of Bangladesh\'s rich archaeological heritage.',
    responsibilities: ['Leading or participating in archaeological excavations.', 'Conserving historical sites and artifacts.', 'Conducting research and publishing findings.'],
    requirements: ['Masters or PhD in Archaeology or Anthropology.', 'Field experience in excavation and conservation.', 'Passion for history and cultural heritage.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 50), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), -20), 'yyyy-MM-dd'),
  },
  {
    id: 'j20',
    title: 'Sub-Registrar',
    department: 'Directorate of Registration',
    location: 'Various Districts',
    type: 'Full-time',
    salaryRange: 'As per National Pay Scale',
    description: 'Responsible for the registration of property deeds and other legal documents at the sub-registry office.',
    responsibilities: ['Registering land sale/purchase deeds.', 'Maintaining official records of registration.', 'Ensuring legal compliance of documents.'],
    requirements: ['LL.B or LL.M degree.', 'Qualified through the Bangladesh Judicial Service Commission (BJSC) examination or equivalent.', 'Meticulous and detail-oriented.'],
    status: 'Open',
    postedDate: format(subDays(new Date(), 28), 'yyyy-MM-dd'),
    applicationDeadline: format(addDays(new Date(), 2), 'yyyy-MM-dd'),
  },
];

export const applications: Application[] = [
    { id: 'app1', jobId: 'j1', candidateId: 'c1', status: 'Interview', applicationDate: format(subDays(new Date(), 1), 'yyyy-MM-dd') },
    { id: 'app2', jobId: 'j2', candidateId: 'c2', status: 'Applied', applicationDate: format(subDays(new Date(), 8), 'yyyy-MM-dd') },
    { id: 'app3', jobId: 'j3', candidateId: 'c3', status: 'Hired', applicationDate: format(subDays(new Date(), 25), 'yyyy-MM-dd') },
    { id: 'app4', jobId: 'j1', candidateId: 'c3', status: 'Screening', applicationDate: format(subDays(new Date(), 1), 'yyyy-MM-dd') },
    { id: 'app5', jobId: 'j5', candidateId: 'c1', status: 'Offered', applicationDate: format(subDays(new Date(), 3), 'yyyy-MM-dd') },
    { id: 'app6', jobId: 'j6', candidateId: 'c2', status: 'Rejected', applicationDate: format(subDays(new Date(), 14), 'yyyy-MM-dd') },
    { id: 'app7', jobId: 'j1', candidateId: 'c2', status: 'Shortlisted', applicationDate: format(subDays(new Date(), 2), 'yyyy-MM-dd') },
    { id: 'app8', jobId: 'j1', candidateId: 'c4', status: 'Applied', applicationDate: format(subDays(new Date(), 3), 'yyyy-MM-dd') },
];


export const activities: Activity[] = [
  {
    id: 'act1',
    user: { name: 'Admin User', avatar: 'https://picsum.photos/seed/admin/100/100' },
    action: 'posted a new job:',
    target: 'Public Relations Officer',
    timestamp: format(subDays(new Date(), 1), 'yyyy-MM-dd HH:mm'),
  },
  {
    id: 'act2',
    user: { name: 'Alice Johnson', avatar: 'https://picsum.photos/seed/alice/100/100' },
    action: 'applied for:',
    target: 'Assistant Director',
    timestamp: format(subDays(new Date(), 1), 'yyyy-MM-dd HH:mm'),
  },
  {
    id: 'act3',
    user: { name: 'Admin User', avatar: 'https://picsum.photos/seed/admin/100/100' },
    action: 'shortlisted',
    target: 'Charlie Brown for IT Officer',
    timestamp: format(new Date(), 'yyyy-MM-dd HH:mm'),
  },
    {
    id: 'act4',
    user: { name: 'Admin User', avatar: 'https://picsum.photos/seed/admin/100/100' },
    action: 'changed status of',
    target: 'Upazila Nirbahi Officer (UNO) to Closed',
    timestamp: format(subDays(new Date(), 2), 'yyyy-MM-dd HH:mm'),
  }
];

export const iifcLogoUrl = "/iifc-logo.png";
